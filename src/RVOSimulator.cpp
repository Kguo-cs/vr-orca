...
namespace RVO {
	...
	RVOSimulator::RVOSimulator(float timeStep, float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed, float prefSpeed, const Vector2 &velocity) : defaultAgent_(NULL), globalTime_(0.0f), kdTree_(NULL), timeStep_(timeStep)
	{
		kdTree_ = new KdTree(this);
...
#pragma omp parallel for
		...
		for (int i = 0; i < static_cast<int>(agents_.size()); ++i) {
			...
			agents_[i]->computeORCALines();
		}

		for (int i = 0; i < static_cast<int>(agents_.size()); ++i) {
			Agent* & agent=agents_[i];

			agent->bestLinePoints_.clear();
			
			for(int j=0;j<agents_[i]->agentNeighbors_.size();j++){

				Agent* & collider=agents_[i]->agentNeighbors_[j];

				if(agent->dict_.count(collider->id_)==0){
					agents_[i]->optimizeORCALines(collider);
				}
				else
				{
					agents_[i]->bestLinePoints_.push_back(agent->dict_[collider->id_]);
				}
			}
...
#pragma omp parallel for
	...
	void RVOSimulator::setAgentDefaults(float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed,float prefSpeed, const Vector2 &velocity)
	{
		if (defaultAgent_ == NULL) {
			defaultAgent_ = new Agent(this);
		}
